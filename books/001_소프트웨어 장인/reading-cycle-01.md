...
- `@Duflow89`
  - 1 재밌어 보인다. 2 내가 하는 일에 영향이 되는 건가. 요거 두 개 인 것 같아요.

> 제너럴리스트의 방향성을 가지고 커리어를 쌓으시는 건가요? 요즘엔 어떤 분야에 관심을 가지고 계시나요?
- `@Duflow89`
  - 따로 뭔가를 공부하는 건 아니고, 일할 때 겸사겸사 같이 합니다. 그러다 보니 일하는 속도가 조금 떨어질 때도 있네요😅
...
- `@Duflow89`
  - 별로 그런거에는 스트레스는 없고.. 그냥 재미없으면 안합니다!
##>
...
- `@Duflow89`
  - 뭐가 되고 싶은 건 없었는데, 하다보니 제네럴리스트가 적성에 맞는 것 같아요. 하나만 하기 싫은 건 아닌데, 산만한 성격이라 그런지 자꾸 옆에 눈이 가네요.
##>

<## @hei5enbug
> 회사에서는 해보고 싶고 재밌는 업무도 있지만 하기 싫고 귀찮은 일을 해야하는 상황도 있으셨을 것 같아요. 그럴 때 도움이 되는 마음가짐이 있으셨나요?
- `@Duflow89`
  - 나무위키를 보다보면, 새상 별 쓸 데 없는 것 까지 타고타고 들어가며 읽게 되는 경우가 많습니다. 재미없는 일도 위키질하듯이, 다른거 찾아보고 또 이해안되는 말 있으면 또 찾고.. 반복하다보면 신기하고 재미있어지는 것 같아요. 요새는 주로 ChactGPT가 요런 부분에서 아주 큰 도움이 되는 것 같아요.
##>
...
- 유지보수 하기 힘들게 코딩하는법이라는 책이 있다던데 그거 생각나네요 ㅎㅎ.., 저도 매니저님이 생각하는 고참개발자의 모습이 궁금한데요, 그건 `@nimkoes` 님이 질문 하셨으니 그거 답변으로 보겠습니다!
...
<## @hei5enbug
> 항상 비슷하게 반복되어 왔던 고민의 본질은 어떤 것이었는지 구체적으로 궁금합니다!
- `@nimkoes`
  - 트렌드 변화
    - **1990 년대** (정적 웹에서 동적 웹으로의 진입) : HTML + CGI + Perl/PHP/ASP로 구성된 웹. 서버는 페이지 생성기였고, 개발은 텍스트 편집기에서 직접 했다.
    - **2000 년대** (MVC 기반 동적 웹의 전성기) : Java (JSP/Servlet), .NET, PHP, Ruby on Rails 등으로 백엔드와 프론트를 분리한 MVC 아키텍처 중심의 서버 중심 웹앱이 주류 였다.
    - **2010 년대** (SPA와 모바일 대응 중심의 웹 개발) : React, Angular, Vue.js 같은 프론트엔드 프레임워크의 부상과 함께, RESTful API + SPA 구조가 일반화 되었다.
    - **2020 년대** (클라우드 네이티브와 AI 중심 개발 환경) : Next.js, TypeScript, serverless, GPT 기반 AI 코딩 도구 (Copilot, Cursor, ChatGPT) 로 자동화와 실시간 협업, 생산성 극대화 지향.
  - 기술이 시대에 따라 끊임없이 진화하고 추상화된 것을 알 수 있습니다.
  - 하지만 기술이 바뀌어도 우리가 해결하고자 하는 ‘문제’는 본질적으로 달라지지 않았는데요. 우리는 언제나 '사용자에게 더 빠르고, 더 편리하며, 더 안전한 경험을 제공하기 위해' 애써왔고, '더 적은 리소스(물적, 인적 자원)로 더 나은 시스템을 만들기 위해' 도구와 패러다임을 바꿔왔습니다.
  - 즉, 기술은 수단이고 우리가 가진 고민은 항상 비슷한 질문에서 출발했다고 생각 했습니다.
    - 어떻게 하면 이걸 더 쉽고 편하고 빠르게 만들 수 있을까?
    - 어떻게 하면 이 시스템을 더 견고하고 변화에 유연하게 대처할 수 있도록 만들 수 있을까?
    - 어떻게 하면 사용자가 정말 원하는 가치를 전달할 수 있을까?
  - 문제 해결의 욕망은 그대로이고, 해결 방법만 바뀌어온 것이라고 생각 했습니다.
  - 결국 제가 생각한 '항상 비슷하게 반복되어 왔던 고민의 본질'이란 문제 해결 욕구, 욕망에 가깝습니다.
##>

<## @duflow89
> 맨저님은 10년전과 비교했을 때, 마음가짐과 방향에서 어떤 차이가 있나요?
- `@nimkoes`
  - 사회 초년생 답게 어딘가에서 '돈을 받고 일을 한다' 는 사실에 만족하며, 주어진 일만 생각했습니다.
  - 주어진 업무 -> 해결해야 하는 일 -> 가능한 빨리 해결해야하는 일
  - 요즘은 주어진 일 뿐만 아니라
    - 내가 하는 일이 어떤 가치를 전하는지
    - 어떤 부작용을 초래하는지 (꼭 부정적인 부작용을 뜻하는 것은 아닙니다.)
  - 등에 대한 고민을 보다 깊이 합니다.
  - '그래서 내가 이걸 지금 왜 하지?', '이 방법이 최선일까?' 고민을 하며, 최소한 내가 손을 댄 영역은 전과 후를 비교했을 떄 더 나아지기를 바라는 마음을 가지고 있습니다.
##>
